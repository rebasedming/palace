...
interface LauncherProps {
  onSubmit: (mode: "presidents" | "bridges") => void;
}
                  ...
                  >
                    {isLoading ? (
                      <>
                        <svg
                          aria-hidden="true"
                          className="w-4 h-4 mr-3 text-gray-200 animate-spin dark:text-white fill-blue-500"
                          viewBox="0 0 100 101"
                          fill="none"
...
const Launcher = ({ onSubmit }: LauncherProps) => {
  ...
  const onSubmitPrompt = async (text: string) => {
    ...
    try {
      ...
      onSubmit(text.includes("bush") ? "presidents" : "bridges");
    } catch (error) {
